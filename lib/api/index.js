...
async function fetchAPI() {
  const json = await fetch(UPTAP_API).then((res) => res.json());
...
export async function getGames(type, limit) {
  ...
  // games.map((game) => (game["name"] = game.name.replace(/Littel/, "Little")));
  ...
  games.map(
    (game) => (game["icon"] = `https://cdn.iwantalipstick.com/gameicon2/png/${game.name}.png`)
  ); // 设置游戏slug
...
export async function getGamesByCategory(genre) {
  ...
  games = games.filter(
    (game) => game.category.toLowerCase().replace(/\./, "") == genre.toLowerCase()
  );
  // console.log(games);
