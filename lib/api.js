...
export async function fetchAPI(query, { variables } = {}) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_V2_URL}/graphql`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_V2_TOKEN}`,
    },
    body: JSON.stringify({
      query,
      variables,
...
export async function getAllGames() {
  const data = await fetchAPI(`
    query {
      games: game (filter: { status: { _eq: "published" } }, limit: -1, sort: "-creation_date") {
        slug
        title
        appid
      }
      total: game_aggregated (filter: { status: { _eq: "published"} }) {
        countDistinct {
          appid
        }
      }
...
export async function getAllGamesByPage(page) {
  ...
  const data = await fetchAPI(
    ...
    query ($limit: Int!, $offset: Int!) {
      games: game (filter: { status: { _eq: "published" } }, limit: $limit, offset: $offset, sort: "-creation_date") {
        slug
        title
        appid
      }
...
export async function getGamesByCategory(slug, limit = -1) {
  const data = await fetchAPI(
    `
    query ($slug: String!) {
      categories: category (filter: { slug: { _eq: $slug } }) {
        slug
        name
        description
      }
      games: game (filter: { category: { slug: { _eq: $slug } }, status: { _eq: "published"} }, limit: ${limit}, sort: "-creation_date") {
        slug
        title
        appid
      }
      total: game_aggregated (filter: { category: { slug: { _eq: $slug } }, status: { _eq: "published"} }) {
        countDistinct {
          appid
        }
      }
...
export async function getGamesByCategoryAndPage(slug, page) {
  ...
  const data = await fetchAPI(
    ...
    query ($slug: String!, $limit: Int!, $offset: Int!) {
      ...
      games: game (filter: { category: { slug: { _eq: $slug } }, status: { _eq: "published"} }, limit: $limit, offset: $offset, sort: "-creation_date") {
        slug
        title
        appid
      }
...
export async function getGameAndRelatedGamesBySlug(slug) {
  ...
  const data = await fetchAPI(
    `
    query ($slug: String!) {
      game: game (filter: { slug: { _eq: $slug }, status: { _eq: "published"} }) {
        slug
        title
        appid
        description
      ...
      related: game (filter: { slug: { _neq: $slug }, status: { _eq: "published"} }, limit: ${limit}, sort: "-featured") {
        slug
        title
        appid
      }
